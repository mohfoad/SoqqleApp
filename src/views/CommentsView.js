...
export default class CommentsView extends Component {
    ...
    onSendButtonClick() {
        ...
        this.setState(prevState => {
            return {
                dataUser: prevState.dataUser.concat({ 'userId': navigationParam.userId, 'comment': this.state.message, 'commentator_id': navigationParam.userId, 'commentator_name': name, 'profileUrl': profileUrl, 'createdAt': date1,'like':[] })
            }
        });
        /* Api calling */
        let arrayParam = { 'userId': navigationParam.userId, 'comment': this.state.message, 'commentator_id': navigationParam.userId, 'commentator_name': name, 'profilePic': profileUrl, 'post_id': navigationParam.postId }
    ...
    renderItem = (item) => {
        ...
        if (dictUserDetail.like.length>0) {
            ...
            btnLikeConfirmation = <TouchableOpacity style={styles.likeImgIcon} onPress={() => this.onLikeButtonClick(dictUserDetail._id, item.index)}>
